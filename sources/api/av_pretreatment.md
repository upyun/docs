...
| start_time        | string  | false | 视频起始处理时间. 格式"HH:MM:SS".
| end_time        | string  | false | 视频结束处理时间. 格式"HH:MM:SS".
| watermark_img     | string  | false | 水印图片路径. 建议使用png格式图片. 暂时不支持外链.                   
...
| watermark_dx      | int     | false | 水印图片横坐标偏移量, 单位px. 当水印位置为NorthEast时, 默认值为-20. 其余情况下默认值为0
| watermark_dy      | int     | false | 水印图片纵坐标偏移量, 单位px. 当水印位置为NorthEast时, 默认值为-15. 其余情况下默认值为0
...
| map_metadata  | boolean | false | 是否保留视频 meta 信息，默认值 `true`         |
| start_time        | string  | false | 音频起始处理时间. 格式"HH:MM:SS".
| end_time        | string  | false | 音频结束处理时间. 格式"HH:MM:SS".
