...
| expiration            | 是   | 请求的过期时间，UNIX UTC 时间戳（秒），建议为 30 分钟                         |
| notify-url            | 否   | 异步通知 URL，见 [通知规则](#notify_return)                               |
| return-url            | 否   | 同步通知 URL，见 [通知规则](#notify_return)                               |
| content-md5           | 否   | 上传文件的 MD5 值，如果请求中没有文件或文件太大计算 MD5 不方便，可以为空 |
| content-secret        | 否   | 文件密钥，用于保护文件，防止文件被直接访问，详见 [Content-Secret 参数说明](/api/rest_api/#Content-Secret)         |
...
| x-gmkerl-thumb        | 否   | 图片预处理参数，详见 [上传同步作图](/cloud/image/#_3)                       |
| x-gmkerl-type         | 否   | 可选值：`get_meta`，获取图片信息；`get_theme_color`，获取图片主题色          |
| apps                  | 否   | 异步预处理任务列表，详见 [异步预处理](#apps)            |
| ext-param             | 否   | 额外参数，详见 [ext-parm](#ext-parm)                  |
...
* 结果信息包括 `code`、`message`、`url`、`time` 和 `sign`(或 `no-sign`) 参数[\[注 4\]](#note4)。如果上传的是图片，还包括 `image-width`、`image-height`、`image-frames` 和 `image-type` 四个参数（备注：这四个参数不参与回调签名）。如果设置了 `apps` 参数，还包括 `task_ids`。

**notify-url 参数说明**

* 如果没有设置 `notify-url`，不发起异步通知。
* 如果设置了 `notify-url`，文件上传完成后，发送 `POST` 请求至 `notify-url` 指定的地址，`POST` 所提交的信息和 `return-url` 中的结果信息相同。
...
如果设置了 `apps` 参数，文件上传完成后，会创建异步处理任务。目前，异步预处理支持**异步图片处理**和**异步音视频处理**服务。
...
- 异步图片处理，详见[上传异步作图](/cloud/image/#_4)；异步音视频处理，详见[音视频处理](/cloud/av/#_8)。
...
异步处理任务完成后，如果存在 `notify_url`（任务中不存在 `notify_url`，使用[上传参数](#upload_args)中的 `notify_url`，如果上传参数中也不存在，不回调），以回调的形式发送处理结果，里面包含相应的 `task_id` 等信息。
