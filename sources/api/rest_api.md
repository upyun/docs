...
其他上传方式在上传结束之后内容就是固定的，只能读取，不能修改，一旦内容发生了改变，只能重新上传同名文件，而追加上传可以在文件后面直接追加内容，适用于不断产生内容的数据形式，比如视频监控、视频直播、日志等。

### 注意事项
* 上传的文件块大小固定为 1M 或者 1M 的整数倍，否则该文件块为此次追加文件的最后一块，此后将无法再继续追加此文件。
* 追加上传的文件，无法进行其他方式覆盖上传，反之也是如此。
...
|          参数           | 必选 |   类型  |                    说明                                 |
|----------------------- |------|---------|-------------------------------------------------------------|
| X-Upyun-Append    | 是   | String  | 值为 `true`， 表示追加数据                         |
