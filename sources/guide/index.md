...
「注册帐号」及其密码将作为 [又拍云用户控制面板](https://console.又拍云.com/#/login/)的登录所用，您需要通过「注册帐号」在 [又拍云用户控制面板](https://console.又拍云.com/#/login/) 进行服务的新建，操作员授权，订单购买等操作。

「操作员」，顾名思义，是用来操作您的服务的，通过使用注册帐号登录 [又拍云用户控制面板](https://console.又拍云.com/#/login/) 后，您可以创建多个操作员，满足不同的管理需求。
...
在您使用注册帐号登录 [又拍云用户控制面板](https://console.又拍云.com/#/login/) 后，您可以创建不同的服务以方便管理你的文件资源，例如存储类型、自主源站服务等。
...
更多服务类型的区别，请参阅 [服务分几种类型，有哪些区别？](http://docs.又拍云.com/faq/#faq)
...
3. 支持通配符，，比如白名单的 `*又拍云.com` 将允许 `www.又拍云.com`、`abc又拍云.com` 等网站访问。
...
具体使用和配置，请参考:[回源鉴权配置](https://blog.又拍云.com/?p=877)   
...
```
http://demo.b0.upaiyun.com/img/又拍云_test.jpg!200
```
这是一个拥有宽度为 200px 的图片。
...
又拍云额外提供了 FTP 的方式，以便用户可以更方便地对服务的资源进行管理。
...
> 在一些 FTP 工具的主机栏中，你可以输入 `ftp://v0.ftp.又拍云.com` ,这样能够根据协议自动识别连接方式。    

![](http://techs.b0.upaiyun.com/img/ftp.png!600)


### API
除了使用简单的 FTP 的方式之外，又拍云还提供了多种不同的 API，方便在不同的编程环境下使用 又拍云的服务，其中包括：
