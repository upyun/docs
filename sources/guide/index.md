...
UPYUN 拥有多种防盗链方式，在空间后台`服务-->选择空间-->功能配置-->防盗链`就能看见这些防盗链的类型。

#### IP 禁止
IP 禁止功能，也就是 IP 黑名单，可以禁止指定的 IP 地址访问空间的文件资源。
具体的操作方式为：

1. 在`服务-->选择空间-->功能配置-->防盗链-->IP 禁用`开启 IP 禁用，点击保存。
...
1. 在`服务-->选择空间-->功能配置-->防盗链-->域名防盗链` 开启白名单/黑名单，点击保存。
...
1. 在`服务-->选择空间-->功能配置-->防盗链-->客户端白名单`开启客户端白名单，点击保存。
