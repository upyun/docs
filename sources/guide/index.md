...
* 文件类型空间：可以上传任意资源，资源将被保存至 UPYUN 的服务器。且文件的外链将具有 CDN 网络支持。
* 图片类型空间：仅允许上传图片类型的文件，较之文件类型空间，图片类空间额外的提供了「[自定义版本](http://segmentfault.com/blog/yunshu_blog/1190000002400065)」功能，且文件的外链将具有 CDN 网络支持。
...
1. 在`后台主页-->选择空间-->防盗链-->IP 禁用`开启 IP 禁用，点击保存。
...
1. 在`后台主页-->选择空间-->防盗链-->域名防盗链` 开启白名单/黑名单，点击保存。
...
1. 在`后台主页-->选择空间-->防盗链-->客户端白名单`开启客户端白名单，点击保存。
...
token 防盗链的生成方式如下：
 ...
 +  在`后台主页-->选择空间-->防盗链-->Token防盗链`开启 Token 防盗链功能, 并且填写一个任意字符串密钥。
