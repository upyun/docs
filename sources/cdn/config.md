...
又拍云 CDN 平台支持视频文件的码率自适应功能，通过自动分析当前视频文件传输的码率，将视频文件的传输速度控制在视频码率范围之内，可以在一定程度上防止高峰期时带宽占用，节省成本。限速举例：一开始不限速，当播放时长超过  3 秒时，进行 1.1 倍码率限速。该功能可支持的配置包括：
...
登陆 [CDN 控制台](https://console.upyun.com/login/)，依次进入：服务管理 > 功能配置 > 性能优化 > 码率自适应，点击【管理】按钮，进入功能配置界面：
...
* 添加规则：点击配置界面右上角的【添加规则】按钮即可添加码率自适应规则，配置界面如下图所示：
...
* 资源路径：也即对请求的 URI 进行匹配，匹配成功则进入限速逻辑，支持 `*` 通配符，例如：
