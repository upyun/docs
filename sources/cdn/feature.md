...
要求将原图设置为访问时返回404，所有jpg、png、gif图片只能加缩略图版本访问，这个如果是在上传时可以增加`Content-Secret`参数进行设置原图保护，但是如果已经上传完后的图片要进行设置，则需要遍历所有图片文件增加这个参数才可以，这样非常麻烦，而且非常耗时，通过rewrite规则则可以轻松搞定
...
要求访问原图自动调整到指定的缩略图去，`nrimg`是自定义的版本号。
