...
* 若需要禁止某些 IP 访问空间文件，则可以使用 IP 禁用
* 支持 * 通配符，如 `10.11.12.*` 将禁止 `10.11.12.0~10.11.12.255` 的 IP 访问

### 域名防盗链

* 白名单：仅允许名单中的域名网站访问空间文件，其他域名网站都不允许访问
* 黑名单：仅禁止名单中的域名网站访问空间文件，其他域名网站都允许访问
...
Token 防盗链适合下载类网站或应用使用，可设置签名过期时间来控制空间文件的访问时限。
...
对于 `云存储空间` 而言，文件的替换和删除操作，我们均会自动提交缓存刷新任务进行全网刷新，特别地，原图的替换和删除，我们会自动将其对应的所有缩略图版本号也进行刷新。正常情况下全网 5 分钟内就能刷新完毕，针对漏刷的情况，可以在管理后台进行手动刷新。

对于 `CDN 空间` 而言，除了能够在管理后台手动刷新外，我们还提供了 [缓存刷新 API 接口](http://docs.upyun.com/api/purge/) 和全网刷新功能。

### 全路径刷新

> 管理后台：工具箱 > 缓存刷新 > 全路径缓存刷新
...
* 只需添加目录路径，无需添加域名地址。比如若要刷新 `http://yourdomain.com/` 域名下所有的文件，则只需添加 `/*` 即可
...
全网刷新顾名思义，我们会对整个空间下所有的 URL 进行缓存刷新操作，即时生效。

重要说明：

* 同一空间每次刷新间隔至少 10 分钟，一天只能刷新 5 次缓存
...
对于动态 CDN 而言，若开启 `参数跟随` 特性后，不同参数对应的缓存也是不同的，因此可以很方便通过参数的变换来绕过缓存，从而达到资源被动刷新的目的。但对于参数不跟随的情况以及静态 CDN 空间来说，不同参数并不会产生不同的缓存拷贝，例如以下两条请求在 UPYUN CDN 均为命中同一个资源：
