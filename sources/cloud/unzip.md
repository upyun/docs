...
| sources_list      | file     	| 否   	| 存放需要被压缩的文件或目录信息的文件，与 `sources` 参数互斥，使用其中一个即可。见「注」        	|

** 注 **

- `sources` 和 `sources_list` 参数指定的压缩文件最大不能超过 10000 个，总文件大小不能超过 1G。
- 当需要被压缩的文件或目录很多时，可以把这些信息放到一个文件里，再把文件 URI 通过 `sources_list` 参数提交，进行处理。特别地，文件需要上传到需要处理的服务名下。
