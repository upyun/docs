...
| 参数              			| 类型   	| 必选	| 说明                                    			|
|---------------------------|-----------|-------|---------------------------------------------------|
| `/scene/<scene>`             | string    | 是		| 窄带高清场景格式。可选 `self_portrait`，`animation`，`film_calm`，`film_dramatic`           	|

** 注 **

- 关于 `scene` 参数， `self_portrait` 适合用户自拍的视频， `animation` 适合动画， `film_calm` 适合内容变化平缓的电影，电视剧，在线教育，访谈节目，综艺节目等， `film_dramatic` 适合内容变化剧烈的电影，比如商业美国大片。
- 窄带高清接口兼容除 `acodec`，`vcodec` 与 `pv` 外的全部视频转码接口参数, 如果遇到罕见的错误, 可尝试移除除 `scene` 外的参数。
