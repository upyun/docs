...
本文档适用于阅读者了解如何接入拇指流量平台，以达到第三方企业应用平台与本平台实现数据交互的目的。主要面对双方（拇指流量平台与企业应用）的数据交互模式、约定、实现方式以及对接口的功能、报文格式以及使用场景进行详细的描述。
...
1.客户联系我司商务完成商务洽谈,我司为客户创建账号

2.我司邮件发送所创建的账户至客户邮箱

3.客户邮件回复出口 IP 与正式回调地址,完成生产环境联调
...
拇指流量平台所有的参数传递都是通过标准的JSON格式提交,所有的请求响应都是通过JSON格式返回。

拇指流量平台需要与企业应用系统进行信息交换与同步，下图为本API提供的接口方法:


|接口名称 |对应方法 |对应方向 |接口描述 |
|-|-|-|-|
|获取Token |getToken	企业 -> |流量平台	|获取Token（后面接口需要） |
|下订单 |createOrder |企业 -> 拇指流量 |提交流量订单 |
|查询订单 |getOrderStatus |企业 -> 拇指流量	|查询订单的充值状态 |
|充值状态回调 |- |	拇指流量 -> 企业 |	拇指流量充值平台将订单充值结果返回给企业的应用系统 |
|号码归属地查询 |getMobileInfo |企业 -> 拇指流量 |手机号信息（运营商、归属地） |
...
-|-|-|-
appkey |鉴权账号，由拇指流量平台提供 |`string` |`true`
appsecret |鉴权密钥，由拇指流量平台提供 |`string` |`true`
...
下单接口，企业客户发起流量包订购请求，拇指流量平台完成合法性校验之后，并向运营商发送下单请求，同步返回下单结果。
...
描述:
  ...
  拇指流量平台将运营商的订单充值结果异步返回给客户，客户接收后返回json格式的{'info':'1'}给流量平台以确认接收到回调,如果拇指流量平台没有接收到正确格式的确认信息，会每隔1分钟再次回调给客户，最多重新回调3次
