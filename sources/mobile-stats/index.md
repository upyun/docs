...
| sign | String | 必须 | 签名，签名规则为：将 `appkey${YourAppKey}custno${YourCustno}requesttime${YourReqestTime}sign${YourSign}` 中`${...}`变量部分替换为实际值，再进行 SHA1 运算。如果请求参数不包含 `requesttime`，计算签名则不需要该值 |
